...
export default function TrustSection() {
  ...
  return (
    <section className="relative max-w-[1920px] mx-auto py-4 sm:py-6 bg-gradient-to-b from-white to-gray-50 dark:from-zinc-900 dark:to-zinc-950">
      <div className={`relative ${getContainerClasses()}`}>
        <FadeContent duration={500} direction="up" distance={10}>
          <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2">
            <span className="text-sm font-medium text-text-tertiary dark:text-gray-500">
              {t('trust.title')}
            </span>
            <div className="flex items-center gap-4 sm:gap-5">
              {techItems.map((item) => (
                <div key={item.name} className="flex items-center gap-1.5 text-text-secondary dark:text-gray-400">
                  {item.icon}
